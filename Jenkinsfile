  ...
  stages {
    ...
    stage('Build and push docker image release') {
      ...
      steps {
        container('docker') {
          script {
            def magento2RepoPublicKey = 'e09972697200676346ab7a6981ac63fa'
            def magento2RepoPrivateKey = string(credentialsId: 'magento2-repo-keys', variable: 'magento2_repo_private_key')
            docker.withRegistry('https://index.docker.io/v1/', 'jenkins-dockerhub') {
              image = docker.build( "$DOCKER_ORG/$APP_NAME:$IMAGE_TAG", '-f Dockerfile.dev .', '--build-arg MAGENTO2_REPO_PUBLIC_KEY=${magento2RepoPublicKey}', '--build-arg MAGENTO2_REPO_PRIVATE_KEY=${magento2RepoPrivateKey}')
              image.push()
            }
          }
        }

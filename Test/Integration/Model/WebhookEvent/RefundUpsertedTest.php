...
class RefundUpsertedTest extends \PHPUnit\Framework\TestCase
{
     ...
     */
    public function testRefundUpserted()
    {
        ...
        foreach ($refundEvents as $refundEvent) {
            $payload = $refundEvent->getPayloadData();

            $refundInfo = $payload['refund'];
            ...
            /** @var Order $order */
            $order = $orderPayments->getFirstItem()->getOrder();
        ...
        $this->assertEquals(
            ...
            count($cardAuthorizationUpsertedEvents)+ count($captureEvents) + count($refundEvents),
            $webhookCollection->count()
        );
    }
}
